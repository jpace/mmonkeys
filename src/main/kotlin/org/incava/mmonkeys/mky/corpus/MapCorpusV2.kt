...
open class MapCorpusV2(
    words: List<String>,
    intToMapSupplier: () -> MutableMap<Int, MutableMap<String, MutableList<Int>>>,
    val stringToListSupplier: () -> MutableMap<String, MutableList<Int>>,
) : Corpus(words) {
    val lengthToStringsToIndices: MutableMap<Int, MutableMap<String, MutableList<Int>>> = intToMapSupplier()
    ...
    init {
        ...
        words.withIndex().forEach { word ->
            lengthToStringsToIndices
                .computeIfAbsent(word.value.length) { stringToListSupplier() }
