...
class GenVsCalc(private val numInvokes: Long = 100_000_000L, private val trialInvokes: Int = 10) {
    ...
    fun profile() {
        ...
        profiler.add("calc " + calc.numSlots) {
            val result = calc.nextRand()
            saveNumber(calcNumbers, result)
        }
        val genNumbers = mutableMapOf<Int, Int>()
        profiler.add("gen " + gen.size) {
            val result = gen.nextRand()
            saveNumber(genNumbers, result.toInt())
        }
